<template>
  <body class="antialiased font-sans text-gray-900 bg-gray-900">
    <div class="overflow-hidden bg-gray-900">
          ...
          <div  class="w-full max-w-xl xl:px-8 xl:w-5/12">
            <div class="relative">
              ...
              <div class="relative bg-white rounded shadow-2xl p-7 sm:p-10">
                ...
                >
                  Tambah Tagihan SPP Murid
                </h3>
                <form @submit="e => addspp(e)" class="register-box bg-white">
                  <div class="mb-1 sm:mb-2 bg-white">
                    <label
                      for="name"
                      class="inline-block mb-1 font-medium bg-white"
                      >Masukan Nominal</label
                    >
                    <input
                      v-model.trim="spp.nominal"
                      placeholder="nominal"
                      type="number"
                  ...
                  <div class="mb-1 sm:mb-2 bg-white">
                    <label
                      for="name"
                      class="inline-block mb-1 font-medium bg-white"
                      >Masukan Tahun Ajaran</label
                    >
                    <input
                      v-model.trim="spp.tahun"
                      placeholder="tahun"
                      type="number"
                      required=""
                      class="flex-grow w-full h-12 px-4 mb-2 transition duration-200 bg-white border border-gray-300 rounded shadow-sm appearance-none focus:border-deep-purple-accent-400 focus:outline-none focus:shadow-outline"
      ...
      <div v-else class="py-8 ">
               ...
               <section class="flex items-center h-full p-16 dark:bg-gray-900 dark:text-gray-900">
       <div class="container flex flex-col items-center justify-center px-5 mx-auto my-8">
           <div class="max-w-md text-center">
               <h2 class="mb-8 font-extrabold text-9xl dark:text-gray-900">
                   <span class="sr-only text-gray-900">Error</span>511
               </h2>
               <p class="text-2xl font-semibold md:text-3xl">Maaf, kita tidak bisa menampilkan halaman ini.</p>
               <p class="mt-4 mb-8 dark:text-gray-900">Hanya admin yang memiliki akses ke halaman berikut</p>
  ...
  const addspp = async e => {
    e.preventDefault();
  
    if (!spp.nominal || !spp.tahun ) {
      
  
    } else {
      store.dispatch('spp/addspp', spp);
